...
export class UserService {
  ...
  try {
    ...
    const user = await this.repo.findByEmail(email)
    if (!user?.email) throw new Error("Email not found in the system")
  ...
  try {
    ...
    const  refreshToken = jwt.verify(token, process.env.JWT_REFRESH_SECRET!) as {
      userId: string;
      role?: string;
    };
    
    // 2. Check token exists in DB
    const result = await this.repo.checkingTokenInDb(refreshToken as any)
     console.log("result", result);
     
    if (!result) throw new Error("Token revoked")

    //rotate token
    await this.repo.rotateToken(refreshToken as any)
